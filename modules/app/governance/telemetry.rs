//! telemetry — high-cardinality dimensional aggregation.
//!
//! Keys samples by `(tenant, protocol, prg, metric_id)` under a
//! cardinality limit, and forwards periodic rollups to the operations
//! export surface.
//!
//! ## Per-step bound
//!
//! `on_sample` handles ONE sample per call; `step` emits at most one
...
pub unsafe fn step(s: &mut Telemetry, sys: &SyscallTable) {
    ...
    unsafe {
        ...
        // If dim_flag == 0, fall back to the payload-hash key.
        // Samples raised by the sibling components this step arrived through
        // the outbox and already went through `on_sample`; this drains the
        // external producers.
        if s.in_ingest >= 0 {
            let mut rec = [0u8; 256];
            loop {
...
/// Fold one metric sample into the keyed table. Dimensional envelopes

...
//! Params: `authority` (`host[:port]`, port 5672 when omitted), `user`,
//!         `pass`, `vhost`.
...
mod authority;
use authority::{Authority, PORT_AMQP};
...
struct AmqpState {
    ...
    phase: APhase,
    /// `net_proto` conn ids are `u16 LE` on every frame, so this is a
    /// `u16`. Held as a `u8`, the `MSG_CONNECTED` tag check reads the
    /// HIGH BYTE of the conn id where the requester tag should be, never
    /// matches, and the handshake never starts — the socket opens and
    /// not one byte is ever sent.
    conn_id: u16,
    tag: u8,
    started_ms: u64,
...
define_params! {
    ...
    // Tag 1 is retired.
    // The peer, `host[:port]`; the port defaults to 5672.
...
pub unsafe extern "C" fn module_new(
    ...
    unsafe {
        ...
        if !s.authority.adopt(PORT_AMQP) {
            let m = b"[amqp] refusing to construct: authority (host[:port]) is required";
...
pub unsafe extern "C" fn module_step(state: *mut u8) -> i32 {
    unsafe {
        ...
        if s.phase == APhase::Disconnected
            && s.draining == 0
            && s.user_len > 0
            && now >= s.retry_at_ms
        {
            ...
            loop {
                ...
                match msg {
                    ...
                    NET_MSG_ERROR => {
                        ...
                        // — on a connect-phase failure the conn_id is
                        // meaningless and the trailing tag is the only
                        // way to know the failure is ours.
                        let pl = core::slice::from_raw_parts(payload, plen);

...
define_params! {
    ...
    // Tag 1 is retired.
    // The broker, `host[:port]`; the port defaults to 9092.
    7, authority, str, 0 => |s, d, len| {
        if len > 0 {
...
pub unsafe extern "C" fn module_new(
    ...
    unsafe {
        ...
        if !s.authority.adopt(PORT_KAFKA) {
            let m = b"[kafka] refusing to construct: authority (host[:port]) is required";

//! MQTT Codec — MQTT 3.1/3.1.1/5.0 packet parser and framer.
//!
//! All envelopes use `[conn_id: u8][bytes...]` as the first byte on every
//! channel. That matches peer_router's cleartext format so the chain
//! peer_router → protocol_router → mqtt_codec → peer_router preserves the
//! client connection identity end-to-end.
//!
//! State is sharded by conn_id in a fixed slot table. Topic aliases,
//! partial-packet buffers, and connection metadata are keyed on conn_id.
//!
//! Packet wire format (after the conn_id envelope byte):
//!   [fixed_header: 1 byte type|flags] [remaining_length: varint 1-4 bytes]
//!   [variable_header + payload]
//!
//! Ports:
//!   in[0]  raw_in       — `[conn_id][tcp bytes]` from protocol_router.mqtt_out
//!   in[1]  responses_in — `[conn_id][session resp envelope]` from session_processor.codec_out
//!   out[0] proposals_out — `[conn_id][session proposal envelope]` to session_processor.codec_in
//!   out[1] frames_out    — `[conn_id][MQTT frame bytes]` to peer_router.client_resp
...
/// limits. Keeping them defined here lets `mqtt_codec` reject
...
pub extern "C" fn module_new(
    ...
    unsafe {
        ...
        dev_log(sys, 3, b"[mqtt] init v2".as_ptr(), 14);
        0
    }
}

/// Emit a `[conn_id][bytes]` payload as a `MSG_CLIENT_FRAME`
/// envelope-framed message to response_mux. Framing matters because
/// `response_mux` is a fan-in merge over four writers (mqtt/amqp/
...
pub extern "C" fn module_step(state: *mut u8) -> i32 {
    unsafe {
        ...
        // ── Decode inbound: [conn_id][MQTT bytes] ────────────────
        //
        // TCP delivers byte streams, not packets. The kernel may split a
        // single MQTT frame across several `channel_read` returns
        // (small reads, partial flush) or coalesce several frames into
        // one (fast publisher, batched). Per-connection `reass_buf`
        // tracks both cases: every read appends to the conn's buffer,
        // and an inner loop pops every complete packet — leaving any
        // trailing partial bytes parked for the next tick.
        for _ in 0..8 {
            ...
            // [tcp chunk]`.
            let (_mtype, plen) = wire::channel_read_msg(sys, s.in_raw, &mut s.in_buf);
            let n = plen as i32;
            if n < 2 { continue; }
            ...
            // claim to honour (mqtt_codec::MAX_PACKET).
        ...
        // ── Encode outbound responses: [conn_id][session env] → MQTT frame
        //
        // session_processor writes via `channel_write_msg`; payload layout
        // (post-envelope-strip) is `[conn_id][proto][pkt_type][flags][body]`.
        // `MSG_SESSION_RESPONSE` is the only msg_type session_processor emits
        // on this channel; other types would indicate a wiring bug.
        if s.in_responses >= 0 {
            for _ in 0..8 {
                let poll = (sys.channel_poll)(s.in_responses, 0x01);
                if poll <= 0 || (poll as u32 & 0x01) == 0 { break; }

                let (mtype, plen) = wire::channel_read_msg(sys, s.in_responses, &mut s.resp_buf);
                dev_log(sys, 3, b"[mqtt] resp rx".as_ptr(), 14);
                if plen < 4 { continue; }
                if mtype != wire::MSG_SESSION_RESPONSE { continue; }
                let n = plen as usize;
                let conn_id = s.resp_buf[0];
                if s.resp_buf[1] != 0 { continue; } // PROTO_MQTT

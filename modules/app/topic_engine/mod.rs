...
impl Subscription {
    const fn zero() -> Self {
        Self {
            tenant: 0, session_slot: 0, pattern: [0; MAX_TOPIC], pattern_len: 0,
            qos: 0, shared_group_id: 0, remote_prg: 0, active: 0,
        }
    }
}
...
impl SharedGroup {
    const fn zero() -> Self {
        Self { group_id: 0, tenant: 0, member_slots: [0; 16], member_count: 0, next_round_robin: 0, active: 0 }
    }
}
...
fn shared_member_index(topic_hash: u64, publish_ordinal: u64, member_count: u8, seed: u32) -> u8 {
    ...
    for byte in topic_hash.to_le_bytes().iter()
...
fn session_has_shared_subscription(
    ...
    for i in 0..MAX_SUBS {
        if subs[i].active == 1
            ...
            && subs[i].shared_group_id == group_id
            && subs[i].session_slot == session
        {
            return true;
        }
    }
...
fn remove_shared_member(
    ...
    for g in 0..MAX_SHARED_GROUPS {
        if shared[g].active == 0 || shared[g].tenant != tenant || shared[g].group_id != group_id {
            continue;
        }

        let mut kept = 0usize;
        for i in 0..shared[g].member_count as usize {
            let member = shared[g].member_slots[i];
            if member != session {
                shared[g].member_slots[kept] = member;
                kept += 1;
            }
        }
        for i in kept..shared[g].member_slots.len() {
            shared[g].member_slots[i] = 0;
        }
        shared[g].member_count = kept as u8;
        if kept == 0 {
            shared[g] = SharedGroup::zero();
        }
        break;
    }
}
...
pub extern "C" fn module_new(
    ...
    unsafe {
        ...
        for i in 0..MAX_SUBS { s.subs[i] = Subscription::zero(); }
        for i in 0..MAX_SHARED_GROUPS { s.shared[i] = SharedGroup::zero(); }
    ...
    unsafe {
        ...
        if s.in_op >= 0 {
            for _ in 0..16 {
                ...
                if 12 + pat_len > plen || pat_len > MAX_TOPIC { continue; }
                ...
                if group_id != 0 {
                    let mut found = false;
                    for g in 0..MAX_SHARED_GROUPS {
                        if s.shared[g].active == 1 && s.shared[g].group_id == group_id && s.shared[g].tenant == tenant {
                            ...
                            if !already_member && (s.shared[g].member_count as usize) < s.shared[g].member_slots.len() {
                                s.shared[g].member_slots[s.shared[g].member_count as usize] = session;
                ...
                if 10 + pat_len > plen || pat_len > MAX_TOPIC { continue; }
                ...
                for i in 0..MAX_SUBS {
                    if s.subs[i].active == 0 || s.subs[i].tenant != tenant { continue; }
                    let pat = &s.subs[i].pattern[..s.subs[i].pattern_len as usize];
                    let topic = &s.buf[8..8 + topic_len];
                    if !wire::mqtt_topic_match(pat, topic) { continue; }
                    match_count += 1;
                    ...
                    if s.subs[i].shared_group_id != 0 {
                        ...
                        if let Some(g) = g_idx {
                            if shared_delivered[g] { continue; }
                            ...
                            if chosen != s.subs[i].session_slot { continue; }
                    ...
                    if s.subs[i].remote_prg == 0 || s.subs[i].remote_prg == s.local_prg {
                        ...
                        if total > s.out_buf.len() { continue; }
                        ...
                        if s.out_deliver >= 0 {
                            ...
                            let written = wire::channel_write_msg(
                                sys, s.out_deliver, wire::MSG_TOPIC_DELIVER,
                                &s.out_buf[..total],
                            );
                            let expected =
                                (wire::ENVELOPE_HDR + total) as i32;
                            if written == expected {
                                s.deliveries = s.deliveries.wrapping_add(1);
                                dev_log(sys, 3, b"[topic] DELIVER".as_ptr(), 15);
                            } else {
                                s.deliveries_failed =
                                    s.deliveries_failed.wrapping_add(1);
                            }
                        }
                    }
                }
                ...
                if egress_prg != s.local_prg && s.out_forward >= 0 {
                    ...
                    if total <= s.out_buf.len() {
                        ...
                        if p > 0 && (p as u32 & 0x02) != 0 {
                            wire::channel_write_msg(sys, s.out_forward, wire::MSG_FORWARD_REQUEST, &s.out_buf[..total]);

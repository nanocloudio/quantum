...
| Module | Domain | Description |
|--------|--------|-------------|
| `protocol` | ingest | Composite of five components. Unlike the other composites these genuinely interact: **router** classifies each connection from its first bytes (up to 16) and pins the verdict, and the dispatch table hands the record straight to the owning codec rather than across a graph edge. **mqtt** — 3.1/3.1.1/5.0 parser and framer with MQTT 5 property support (subscription identifiers, topic aliases, user properties, reason strings). **kafka** — binary protocol codec with request/response correlation and API-version negotiation. **amqp** — 0-9-1 frame parser: Connection/Channel/Queue/Basic/Confirm methods, content header/body reassembly. Responses arrive on one shared bus, are demuxed by the dispatch table on the session's protocol tag, and are encoded onto one shared `frames_out`. **anchor** is the Fluxor transport-anchor role: it mints a session id per connection, attaches it to a session worker, holds its envelopes across a worker swap and admits the worker's export by its delivery cursors ([architecture/session_continuity.md](architecture/session_continuity.md)). Variants `full` (default) / `mqtt` / `kafka` / `amqp` compile out the codecs a deployment does not serve. |
...
| Module | Domain | Description |
|--------|--------|-------------|
| `session_processor` | apply | The unified session state machine for all protocols — quantum's core application module and the Raft apply consumer that plugs into `consensus`. Decomposed into five components — **sessions** (the session arena and per-session QoS inflight), **store** (the protocol-neutral durable-publish and message-log primitive shared by Kafka and AMQP), **consumers** (consumer groups, offsets, AMQP push consumers), **correlate** (correlation tables and the commit-gating stash), and **worker** (the SessionCtrlV1 session-worker role behind the `protocol` anchor, [architecture/session_continuity.md](architecture/session_continuity.md)) — stepped in a fixed phase sequence; the component split and the rules each enforces are in [architecture/session_decomposition.md](architecture/session_decomposition.md). Variants `full` (default) / `mqtt` / `kafka` / `amqp` compile out the protocols a deployment does not serve. |
...
- [architecture/session_continuity.md](architecture/session_continuity.md) — `protocol` as the Fluxor transport anchor and `session_processor` as the session worker: the identity rule, the seam and its cursors, the swap, and the gates

...
#[derive(Debug, Parser)]
#[command(about = "Seed deterministic workload data from fixtures.", version)]
struct Args {
    ...
    data_root: PathBuf,
    /// Limit seeding to one or more tenants (repeat flag to specify multiple tenants).
    #[arg(long, action = ArgAction::Append)]
    tenant: Vec<String>,
    /// If set, validate fixtures but do not write any files.
    #[arg(long)]
    dry_run: bool,
}
